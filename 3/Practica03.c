...
// Máquina de estados de los inputs
Estado estado = 0;
Estado estado_anterior = 8;

// LEDs
uint16_t ms_elapsed = 0; // Cuenta los ms que han pasado desde el último cambio de un LED al siguiente

// Reloj
uint32_t time_seconds = 0; // Segundos desde las 00:00: El rango es [0, 86399]
bool time_running = 1; // Bool que indica si el reloj est� pausado o no

// Alarma
uint16_t alarm_minutes = 0; // Minuto en el que sonará la alarma. El rango es [0, 1439]

// Control LEDs P7
const uint32_t INC_RETRASO = 10; // Incremento/decremento en ms del tiempo entre cambio de LEDs al pulsar up/down
const uint32_t MAX_RETRASO = 5000; // Tiempo máximo en ms entre cambio de LEDs
const uint32_t MIN_RETRASO = 10; // Tiempo mínimo en ms entre cambio de LEDs
...
void manage_states(Estado estado, bool *izquierdaderecha, uint16_t *retraso_leds, uint8_t *selected_field) {
    ...
    case UP: // Up
        ...
        *retraso_leds =
                ...
                (retraso_temp > MAX_RETRASO) ? // Controlamos el valor m�nimo permitido
    ...
    case DOWN: // Down
        ...
        *retraso_leds =
                ...
                (retraso_temp < MIN_RETRASO) ? // Controlamos el valor m�nimo permitido
 ...
 * Este timer se usar� para el delay de los leds
 **/
 ...
 * Este timer se usar� para el reloj y la hora
 **/
void TA1_0_IRQHandler(void) {
    TA1CCTL0 &= ~CCIE; // Desactivamos interrupciones

    time_seconds++; // Ha pasado 1s
    time_seconds %= 86400; // Un d�a = 86400s
